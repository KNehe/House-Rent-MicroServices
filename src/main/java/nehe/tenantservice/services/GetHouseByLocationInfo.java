...
@Service
public class GetHouseByLocationInfo {

    private HouseServiceProxy houseServiceProxy;

    @Autowired
    public GetHouseByLocationInfo(HouseServiceProxy houseServiceProxy) {
        this.houseServiceProxy = houseServiceProxy;
    }

    @HystrixCommand(fallbackMethod = "getFallBackHouseByLocation",
            threadPoolKey = "locationPool",
    ...
    public ResponseEntity<?> getFallBackHouseByLocation(@PathVariable String location){
